...
def constraintFromForeignKeyFactory(r1, xxx_todo_changeme, scope={}):
    ...
    def constraintFromForeignKey():
        return eval(r2, globals(), scope)(list(map.values())) >= r1(list(map.keys())).rename(map)(list(map.values()))
...
class Tuple(dict):
    ...
    def __init__(self, _indict=None, **args):
        ...
        myhash = 0
        for it in self.items(): myhash ^= hash(it)
        self.myhash = myhash
    ...
    def __repr__(self):
        ...
        return "Tuple(%s)" % ", ".join(["%s=%s" % (k,repr(v)) for (k,v) in list(self.items())])
...
class Relation(object):
    ...
    def _recalc_hash(self):
        myhash=0
        if isinstance(self._body, collections.Callable):
    ...
    def setBody(self, body):
        ...
        if isinstance(body, collections.Callable):
            self._body=body
        ...
        self._headingInvert=dict(list(zip(self._heading, [{} for attr in self._heading])))
    ...
    def _scan(self, rel=None):
        ...
        if rel is None:
            if isinstance(self._body, collections.Callable):
                if self._body.__code__.co_argcount == 0 or (self._body.__code__.co_argcount == 1 and isinstance(self._body, types.MethodType)):   #e.g. a view (function which returns a Relation, i.e. complete set) #todo unless recursive!
    ...
    def __str__(self):
        columns = range(len(self._heading))
        ...
        def line(row):
            ...
            if len(vals)>0:
                for r in range(max([len(n) for n in list(vals.values())])):
                    lr = []
                    for col in self._heading:
                        if r < len(vals[col]):
                            value = vals[col][r]
    ...
    def __repr__(self):
        ...
        sbody=None
        if isinstance(self._body, collections.Callable):
            if not (self._body.__code__.co_argcount == 0 or (self._body.__code__.co_argcount == 1 and isinstance(self._body, types.MethodType))):   #e.g. a view (function which returns a Relation, i.e. complete set) #todo unless recursive!
        ...
        return "Relation(%(heading)s,\n%(body)s,\n%(constraints)s)" % {'heading':str(self._heading),
                                                                        'body':sbody.replace('{','\n{'),                                                                        
                                                                        'constraints':"{%s}" % ",\n".join(["'%s':(%s, %s)" % (cname, _convertToShorthand(kn), p) for cname, (kn, p) in list(self.constraint_definitions.items())])}


    def toTuple(self):
        """Converts a relation to a Tuple (assumes 1 tuple in relation)"""
        if not isinstance(self._body, collections.Callable) and len(self._body)!=1:
    ...
    def fromTuple(tr, constraints=None):
        ...
        return Relation([k for k in tr], [tuple([v for v in list(tr.values())])], constraints)
    ...
    def update(self, restriction = lambda r:True, Hsetting = [], setting = lambda t:{}):
        ...
        t1 = old.rename(dict(list(zip(self._heading, ['_OLD_'+x for x in self._heading]))))
        ...
        new = upd.remove(Hupdold).rename(
                           dict(list(zip([x for x in Hupdback if x.startswith('_OLD_')], [x.lstrip('_OLD_') for x in Hupdback if x.startswith('_OLD_')]))))
...
def AND(r1, r2):
    ...
    """
    #Optimise the order
    if isinstance(r1._body, collections.Callable):
        if not isinstance(r2._body, collections.Callable):
            return AND(r2, r1)
        else:
            if not(r1._body.__code__.co_argcount == 0 or (r1._body.__code__.co_argcount == 1 and isinstance(r1._body, types.MethodType))):   #e.g. a view (function which returns a Relation, i.e. complete set) #todo unless recursive!
                if r2._body.__code__.co_argcount == 0 or (r2._body.__code__.co_argcount == 1 and isinstance(r2._body, types.MethodType)):   #e.g. a view (function which returns a Relation, i.e. complete set) #todo unless recursive!
                    return AND(r2, r1)
                else:
...
def RESTRICT(r, restriction = lambda trx:True):
    """Restrict rows based on condition
       ...
       (macro)"""
    if not isinstance(restriction, collections.Callable):
...
def EXTEND(r, Hextension=[], extension = lambda trx:{}):
    """Extend rows based on extension
       ...
       (macro)"""
    if not isinstance(extension, collections.Callable):
...
def SUMMARIZE(r1, r2, exps):
    ...
    return REMOVE(EXTEND(r, list(exps.keys()), lambda u:dict(list(zip(list(exps.keys()),
                                          [agg(u["_Y"], exp)
                                           for (agg, exp) in list(exps.values())])))),
                  ["_Y"]
                 )
...
def UNGROUP(r, groupname):
    ...
    T=Relation([groupname]+list(Hs),
               [dict(list(zip([groupname]+list(t.keys()), [s[groupname]]+list(t.values()))))
...
def QUOTA(r, limit, Hr=None, asc=True):
    ...
    return REMOVE(
              EXTEND(r, ['_higher'],
                    lambda t:{'_higher':COUNT( r.rename( dict(list(zip(Hr, ['_'+x for x in Hr]))) ).where(
...
def SUM(r, expression = lambda trx:None):   #todo None ok? cause error = good?
    """Sum expression"""
    if expression.__code__.co_consts==(None,) and len(r._heading)==1:     #if no expression but 1 attribute, use it
