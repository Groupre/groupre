...
def front_fallback(preference, chair):
        ...
        if 'front' in preference.name:
            ...
            if has_attribute:
                while (not preference_found
                    ...
                    if groupre_globals.FALLBACK_CHAIRS_FRONT[fallback_level] in chair.attributes:
...
def back_fallback(preference, chair):
        ...
        if 'back' in preference.name:
            ...
            if has_attribute:
                while (not preference_found
                    ...
                    if groupre_globals.FALLBACK_CHAIRS_BACK[fallback_level] in chair.attributes:
                        score += ((groupre_globals.FALLBACK_LIMIT_BACK - fallback_level + 1)
                                  / (groupre_globals.FALLBACK_LIMIT_BACK + 1))
                        preference_found = True
                    else:
...
def aisle_fallback(preference, chair):
        ...
        if 'aisle' in preference.name:
            ...
            if has_attribute:
                while (not preference_found
                    ...
                    if groupre_globals.FALLBACK_CHAIRS_AISLE[fallback_level] in chair.attributes:
                        score += ((groupre_globals.FALLBACK_LIMIT_BACK - fallback_level + 1)
                                  / (groupre_globals.FALLBACK_LIMIT_BACK + 1))
                        preference_found = True
                    else:
...
def priority_match(student, chairs, team_fields, team_structures):
    ...
    if groupre_globals.FALLBACK_ENABLED and max_score == 0:
        ...
        for chair in chairs:
            ...
            for preference in student.preferences:
                score += front_fallback(preference, chair)
                score += back_fallback(preference, chair)
                score += aisle_fallback(preference, chair)
